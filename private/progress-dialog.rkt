...
(define progress-dialog%
  (class dialog%
    ...
    (define gauge
      (new gauge% [parent this] [label #f] [range 100] [min-width 320]))
    (define is-cancelled #f)

    (when cancellable
      (new button% [parent this] [label "Cancel"]
           [callback (λ (_b _e) (set! is-cancelled #t) (on-cancel))]))
