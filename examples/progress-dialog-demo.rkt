...
(define f
  (new frame% [label "progress-dialog% demo"] [width 360] [height 110] [alignment '(center center)]))

(new message% [parent f] [label "A modal progress dialog runs on launch; click Run to repeat."])
(new button% [parent f] [label "Run"] [callback (λ (_b _e) (run))])
...
(define (run)
  ...
  (void (thread (λ ()
                  (for ([i (in-range 0 101 5)])
                    ...
                    (queue-callback (λ ()
                                      (send pd set-progress i)
                                      (send pd set-message (format "~a%" i))))
                    (sleep 0.08))
                  (unless (send pd cancelled?)
                    (queue-callback (λ ()
                                      (send pd set-progress 100)
                                      (send pd set-message "Done."))))
                  (sleep 0.3)
