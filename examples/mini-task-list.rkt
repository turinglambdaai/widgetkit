...
(define f (new frame% [label "widgetkit — mini task list"] [width 500] [height 440]
               [alignment '(left top)]))
...
(define field (new labeled-field% [parent input-row] [label #f]
                   [cue "Add a task and press Add"] [tooltip "Type a task name"]))
(new button% [parent input-row] [label "Add"]
     [callback (λ (_b _e) (add-task))])

;; --- The task list --------------------------------------------------------
(define tasks '())   ; list of strings
(define lst (new text-list% [parent f] [items #()] [item-height 22] [min-height 220]
                 [action (λ (item) (status (format "Selected: ~a" item)))]))

;; --- Collapsible actions --------------------------------------------------
(define actions (new disclosure% [parent f] [label "Actions"] [expanded? #f]))
(new button% [parent (send actions get-content)] [label "Remove selected"]
     [callback (λ (_b _e) (remove-selected))])
(new button% [parent (send actions get-content)] [label "Clear all"]
     [callback (λ (_b _e) (clear-all))])

;; --- Status bar -----------------------------------------------------------
(define bar (new status-bar% [parent f] [initial-message "Ready."]))
(define (status msg) (send bar set-message msg))
...
(define (add-task)
  ...
  (cond
    ...
    [else (set! tasks (append tasks (list t)))
...
(define (remove-selected)
  ...
  (cond
    ...
    [else (set! tasks (for/list ([t tasks] [i (in-naturals)] #:unless (= i idx)) t))
          (refresh)
          (status "Removed.")]))
